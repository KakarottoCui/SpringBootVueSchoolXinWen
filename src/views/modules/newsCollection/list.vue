        ...
        <div v-if="showFlag">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('newsCollection','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/xiaoyuanxinwen/upload/newsCollectionMuBan.xls"
                        >批量导入新闻收藏数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('newsCollection','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="newsCollectionUploadSuccess"
                                :on-error="newsCollectionUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('newsCollection','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "newsCollection.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="newsCollectionTypes"
                                      header-align="center"
                                      label="类型">
                        <template slot-scope="scope">
                            {{scope.row.newsCollectionValue}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('newsCollection','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('newsCollection','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('newsCollection','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'news_collection',//当前表表名,
                        sumColum : 'news_collection_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'news_collection_name',//分组字符串字段
                        // types : 'news_collection_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "news_collection_types",
                //// riqi :getMonthFormat(_this.echartsDate),//年月
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                if (this.searchForm.newsErjiTypes!= '' && this.searchForm.newsErjiTypes!= undefined) {
                    params['newsErjiTypes'] = this.searchForm.newsErjiTypes
                }
                ...
                params['newsCollectionDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "newsCollection/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
                ...
                this.$http({
                    url: "dictionary/page?dicCode=news_erji_types&page=1&limit=100",
                    method: "get",
                    page: 1,
                    limit: 100,
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.newsErjiTypesSelectSearch = data.data.list;
                    }
                });
                this.$http({
            ...
            // 导入功能上传文件成功后调用导入方法
            newsCollectionUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "newsCollection/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            // 导入功能上传文件失败后调用导入方法
            newsCollectionUploadError(data){
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "newsCollection/update",
                        method: "post",
                        data: {
                            id:id,
//                            newsCollectionTypes:newsCollectionTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
